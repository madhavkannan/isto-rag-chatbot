...
**Story 4 — general policy Q&A, no tool call at all** (`"Does doing an
internship impact full-time work?"`)
Unlike Stories 1-2, this one is answered entirely from retrieved policy
text (`lambda/kb_seed/policy_docs.py`'s work-authorization entries), with
no DynamoDB lookup and no deterministic verdict behind it — a deliberate
contrast to the rest of the demo. The assistant has to synthesize across
two separate policy excerpts (full-time work authorization's rules, and
internship authorization's 12-month threshold) rather than look up a
precomputed answer or paste one excerpt back verbatim. It's also the one
place in the demo where an output-grounding check would have real work
to do, unlike Stories 1-2 where the verdict is already data before the
model speaks — deliberately left out of scope for this demo.
