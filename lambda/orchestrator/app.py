...
breakdown if asked live):
  ...
  4. Call the model via Bedrock Converse (bedrock_client.py), with Bedrock
     Guardrails attached and the Story 1 (travel) and Story 2 (course drop
     / Medical RCL) tools available.
        ...
        if evaluation.needs_escalation:
            if not evaluation.attendance.compliant and evaluation.signature.status != "ok":
                instructions.append(
                    ...
                    "Order matters here — state the verdict FIRST, its own "
                    "line, right after you say what you checked (e.g. "
                    "'Your trip doesn't work as requested.'): do NOT put "
                    "the verdict after the options, and do NOT save it for "
                    "the closing line here even though that's the usual "
                    "pattern elsewhere. Only after that verdict, present "
                    "the choice as two separate, clearly labeled bullets — "
                    "'- **Option A:** ...' and '- **Option B:** ...' — "
                    "never as a single run-on sentence. Option A is the "
                    "recommended compliant dates (case is just a signature "
                    "renewal). Option B is their original requested dates "
                    "(same case, but it also requests an attendance "
                    "exception — make clear that's ISTO's call, not "
                    "guaranteed). Close by asking which option they'd "
                    "like — that question is the closing line, not a "
                    "restated verdict. Wait for their choice, then call "
